...
// ==================== CUSTOM HOOK ====================
export function useCustomerInvoice({ useMock = true }: { useMock?: boolean } = {}) {
  ...
  // Payment modal
  const openPaymentModal = useCallback((invoice: CustomerInvoice) => {
  ...
  // Download handler
  const handleDownload = useCallback((invoice: CustomerInvoice) => {
    console.log('Downloading invoice:', invoice.id);
    alert(`Downloading invoice ${invoice.id} as PDF`);
  }, []);

  // Print handler
  const handlePrint = useCallback((invoice: CustomerInvoice) => {
